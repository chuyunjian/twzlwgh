...
namespace Learun.Application.Organization
{
    ...
    public class DepartmentBLL : DepartmentIBLL
    {
        ...
        #region 获取数据
        /// <summary>
        /// 获取过滤了权限的部门列表信息(根据公司Id)
        /// </summary>
        /// <param name="companyId">公司Id</param>
        /// <param name="filter">是否过滤权限，默认是</param>
        /// <returns></returns>
        public List<DepartmentEntity> GetList(string companyId, bool filter = true)
        {
            try
            {
                List<DepartmentEntity> list = cache.Read<List<DepartmentEntity>>(cacheKey + companyId + filter.ToString(), CacheId.department);
                if (list == null || list.Count == 0)
                {
                    list = (List<DepartmentEntity>)departmentService.GetList(companyId, filter);
                    cache.Write<List<DepartmentEntity>>(cacheKey + companyId + filter.ToString(), list, CacheId.department);
                }
                return list;
            }
            catch (Exception ex)
        ...
        /// <param name="filter">是否过滤权限，默认是</param>
        /// <returns></returns>
        public List<TreeModel> GetTree(string companyId, string parentId, bool filter = true)
        {
            try
            {
                ...
                List<DepartmentEntity> list = GetList(companyId, filter);
        ...
        /// <returns></returns>
        public List<TreeModel> GetTree(List<CompanyEntity> companylist, bool filter = true)
        {
            try
            {
                List<TreeModel> treeList = new List<TreeModel>();
                ...
                {
                    List<TreeModel> departmentTree = GetTree(companyone.F_CompanyId, "", filter);
        ...
        public List<string> GetSubNodes(string companyId, string parentId)
        {
            try
            {
                ...
                res.Add(parentId);
                List<TreeModel> list = GetTree(companyId, parentId, false);
        ...
        public void VirtualDelete(string keyValue)
        {
            try
            {
                ...
                cache.Remove(cacheKey + entity.F_CompanyId + "True", CacheId.department);
                cache.Remove(cacheKey + entity.F_CompanyId + "False", CacheId.department);
        ...
        public void SaveEntity(string keyValue, DepartmentEntity departmentEntity)
        {
            try
            {
                ...
                cache.Remove(cacheKey + "dic", CacheId.department);
                cache.Remove(cacheKey + departmentEntity.F_CompanyId + "True", CacheId.department);
                cache.Remove(cacheKey + departmentEntity.F_CompanyId + "False", CacheId.department);
