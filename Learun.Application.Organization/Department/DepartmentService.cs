...
namespace Learun.Application.Organization
{
    ...
    public class DepartmentService : RepositoryFactory
    {
        ...
        /// <returns></returns>
        public IEnumerable<DepartmentEntity> GetList(string companyId, bool filter = true)
        {
            try
            {
                ...
                {
                    strSql.Append(" AND F_CompanyId = @companyId");
                }
                else if (!filter)
                {
                    strSql.Append(" AND F_CompanyId = '-999'");
                }
                //过滤权限
                if (filter)
                {
                    Learun.DataBase.Util.DataPermission.AppendSql(strSql);
                }
