/*
 * 日 期：2018-12-14
 * 描 述：新增了一些自定义的插件，单独建立，防止升级版本无法使用
 */
(function ($, learun) {
    "use strict";
    ...
    $.fn.lrGetFormData = function (keyValue) {// 获取表单数据
        var resdata = {};
        $(this).find('input,select,textarea,.lr-select,.lr-formselect,.lrUploader-wrap,.sbdUploader-wrap,.lr-radio,.lr-checkbox,.edui-default').each(function (r) {
            var id = $(this).attr('id');
            if (!!id) {
                ...
                switch (type) {
                    ...
                    case "lr-Uploader":
                        ...
                        break;
                    case "sbd-Uploader":
                        resdata[id] = $(this).sbdUploaderGet();
                        break;
                    case "lr-radio":
    ...
    $.fn.lrDataItemSelect = function (op) {
        ...
        var dfop = {
            ...
            // 访问数据接口参数
            param: { itemCode: '', parentId: '0', type: 'default' },
            // 级联下级框
        }
        op = op || {};
        if (!op.code) {
        ...
        dfop.param.placeholder = op.placeholder || '';
    ...
    // 数据源下拉框加条件
    $.fn.lrDataSourceSelectByWhere = function (op) {
        op = op || {};
        var dfop = {
            // 是否允许搜索
            allowSearch: true,
        ...
        learun.clientdata.getAllAsync('sourceDataByWhere', {
            code: op.code,
            query: op.query,
            callback: function (dataes) {
                $select.lrselectRefresh({
    ...
    $.fn.lrCompanySelect = function (op) {
        ...
        var dfop = {
            ...
            // 访问数据接口参数
            param: { parentId: '0', filter: true, nature: "" },
        };
        op = op || {};
        dfop.param.parentId = op.parentId || '0';
        dfop.param.filter = op.filter != null && typeof (op.filter) != "undefined" ? op.filter : true;
        dfop.param.nature = op.nature || '';
        if (!!op.isLocal) {
            dfop.url = '';
        }
        var $select = $(this).lrselect(dfop);
    ...
    $.fn.lrDepartmentSelect = function (op) {
        ...
        var dfop = {
            ...
            // 访问数据接口参数
            param: { companyId: '', parentId: '0', showcheck: '', filter: true },
        ...
        dfop.param.showcheck = op.showcheck;
        dfop.param.filter = op.filter;

        return $(this).lrselect(dfop);;
    };
    // 党组织下拉框
    $.fn.lrDJDepartmentSelect = function (op) {
        // op:parentId 父级id,maxHeight 200,
        ...
        var dfop = {
            ...
            allowSearch: true,
            // 访问数据接口地址
            url: top.$.rootUrl + '/MD_Code/DJ_Department/GetTree',
            // 访问数据接口参数
    ...
    // 数据源下拉树选择
    $.fn.lrDataSourceTree = function (op) {
        // op:parentId 父级id,maxHeight 200,
        var type = 'tree';
        ...
        var dfop = {
            ...
            allowSearch: true,
            // 访问数据接口地址
            url: top.$.rootUrl + '/LR_SystemModule/DataSource/GetTree',
            // 访问数据接口参数
            param: { code: '', parentId: '0', Id: '', showId: '', showcheck: false },
        }
        op = op || {};
        dfop.param.code = op.code;
        dfop.param.parentId = op.parentId;
        dfop.param.Id = op.Id;
        dfop.param.showId = op.showId;
        dfop.param.showcheck = op.showcheck;
        return $(this).lrselect(dfop);;
    };
    // 人员下拉框
    ...
    // 省市区级联下拉树选择
    $.fn.lrAreaTreeSelect = function (op) {
        ...
        var dfop = {
            ...
            // 访问数据接口参数
            param: { parentId: top.$.config.AreaID, childNodes: false, showcheck: false }
        }
        op = op || {};
        dfop.param.placeholder = op.placeholder || '';
        dfop.param.parentId = op.parentId || '0';
        dfop.placeholder = op.placeholder;
        dfop.param.areaid = op.areaid;
        return $(this).lrareaselect(dfop);
    };
    //大小类级联
    $.fn.lrTypeSelect = function (op) {
        // op:parentId 父级id,maxHeight 200,
        var dfop = {
            // 字段
            value: "value",
            ...
            allowSearch: true,
            // 访问数据接口地址
            url: top.$.rootUrl + '/DSJ_Code/YZ_Types/GetTree',
            // 访问数据接口参数
            param: { parentId: '', mark: '0' },
        }
        op = op || {};
        if (!!op.parentId) {
            dfop.param.parentId = op.parentId;
            ...
            if (i == 0) {
                ...
                _obj[i - 1].on('change', function () {
                    ...
                    if (_value == "") {
                        ...
                        $obj.lrselectRefresh({
                            url: top.$.rootUrl + '/DSJ_Code/YZ_Types/GetTree',
                            param: { parentId: _value, mark: op.mark },
                        });
                    }
