...
namespace Learun.Application.Base.SystemModule
{
    ...
    public class DataItemService : RepositoryFactory
    {
        ...
        public void SaveClassifyEntity(string keyValue, DataItemEntity entity) {
            IRepository db = this.BaseRepository();
            try
            {
                db.BeginTrans();
                if (string.IsNullOrEmpty(keyValue))
                {
                    ...
                    db.Update(entity);
                    //假如修改了当前不是树型，就修改数据字典详细的父级编号为0
                    if (entity.F_IsTree == 0)
                    {
                        string sql = " UPDATE LR_Base_DataItemDetail set F_ParentId=0  WHERE F_ItemId=@F_ItemId";
                        db.ExecuteBySql(sql,new { entity.F_ItemId});
                    }
                }
                db.Commit();
            }
