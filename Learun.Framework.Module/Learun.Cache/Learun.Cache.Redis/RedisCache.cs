...
namespace Learun.Cache.Redis
{
    ...
    public class RedisCache
    {
        ...
        /// </summary>
        /// <returns></returns>
        private static CSRedis.CSRedisClient CreateCSRedisClient(long dbId)
        {
            string[] writeServerList = SplitString(redisConfigInfo.WriteServerList, ",");
            string[] readServerList = SplitString(redisConfigInfo.ReadServerList, ",");
            CSRedis.CSRedisClient csredis = null;
            if (writeServerList.Count() == 1)
            {
                //普通模式
                csredis = new CSRedis.CSRedisClient(writeServerList[0] + ",defaultDatabase=" + dbId);
            }
            return csredis;
        }
        /// <summary>
        ...
        public static bool Remove(string key, long dbId = 0)
        {
            ...
            {
                var redis = CreateCSRedisClient(dbId);
                var keys = redis.Keys(key);
                var res = redis.Del(keys);
                redis.Dispose();
        ...
        /// 清空所有缓存
        /// </summary>
        public static void RemoveAll(long dbId = 0, string RedisPrev = "")
        {
            var redis = CreateCSRedisClient(dbId);
            var keys = redis.Keys(RedisPrev + "*");
            redis.Del(keys);
            redis.Dispose();
        }
        #endregion
