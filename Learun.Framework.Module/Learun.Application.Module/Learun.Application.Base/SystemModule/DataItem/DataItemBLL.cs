...
namespace Learun.Application.Base.SystemModule
{
    ...
    public class DataItemBLL : DataItemIBLL
    {
        ...
        public string GetDataItemTextByCodeAndValue(string code, string value)
        {
            string F_ItemName = string.Empty;
            DataItemIBLL bll = new DataItemBLL();
            var list = bll.GetDetailList(code);
            if (list == null)
                return "";
            ...
            {
                var info = list.Where<DataItemDetailEntity>(t => t.F_ItemValue.Equals(item)).FirstOrDefault<DataItemDetailEntity>();
                if (info != null)
                    namelist.Add(info.F_ItemName);
            }

            F_ItemName = string.Join(",", namelist);
            return F_ItemName;
        }

        #endregion
        #region 通过数据词典的text得到对应的Value
        /// <summary>
        ...
        public string GetDataItemValueByCodeAndText(string code, string text)
        {
            DataItemIBLL bll = new DataItemBLL();
            var list = bll.GetDetailList(code);
            if (list == null)
                return "";
            var info = list.Where<DataItemDetailEntity>(t => t.F_ItemName.Equals(text)).FirstOrDefault<DataItemDetailEntity>();
            if (info == null)
                return "";
            return info.F_ItemValue;
        }
