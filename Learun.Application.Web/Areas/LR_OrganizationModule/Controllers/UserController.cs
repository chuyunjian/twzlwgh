...
namespace Learun.Application.Web.Areas.LR_OrganizationModule.Controllers
{
    ...
    public class UserController : MvcControllerBase
    {
        ...
        /// <returns></returns>
        [HttpGet]
        public ActionResult SelectRoleForm()
        {
            return View();
        }
        #endregion
        ...
        public ActionResult GetPageList(string pagination, string keyword, string companyId, string departmentId)
        {
            ...
            var roles = roleIBLL.GetList();
            //用户角色
            UserRelationIBLL userRelationIBLL = new UserRelationBLL();
            foreach (var item in data)
            {
                var relation = userRelationIBLL.GetObjectIdList(item.F_UserId, 1);
                if (relation.Count > 0)
                {
                    item.RoleName = string.Join("、", roles.Where(a => relation.Any(c => c.F_ObjectId == a.F_RoleId)).Select(a => a.F_FullName));
                }
            }
            var jsonData = new
            {
        ...
        [AjaxOnly]
        public ActionResult SaveForm(string keyValue, UserEntity entity, string RoleIDs)
        {
            userIBLL.SaveEntity(keyValue, entity);
            //保存角色
            UserRelationIBLL userRelationIBLL = new UserRelationBLL();
            userRelationIBLL.SaveObjectList(RoleIDs, 1, entity.F_UserId);
            return Success("保存成功！");
        }
        /// <summary>
        /// 删除表单数据
