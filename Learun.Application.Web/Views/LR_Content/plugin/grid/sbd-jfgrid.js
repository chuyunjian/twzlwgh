(function ($, learun) {
                        ...
                        rowItem.$cell.find('.fa-ellipsis-h').on('click', function () {
                            ...
                            if (item.edit.op) {
                                learun.layerForm({
                                    id: 'lrgridlayerform',
                                    title: '选择' + item.label,
                                    //用法：不传formUrl调用数据字典，传入调自定义页面    重写原因：需求弹出一个自定义的form页面
                                    url: top.$.rootUrl + (item.edit.op.layerUrl ? item.edit.op.layerUrl : '/Utility/JfGirdLayerForm'),
    ...
    var _jfgrid = {
        init: function ($self, op) {
            ...
            //添加自定义的class
            if (typeof (op.class) != "undefined") {
                $self.addClass(op.class);
            }
            // 添加一个移动板
            if ($jfgridmove === null) {
        ...
        expandSub: function ($self, $et, op) {
            //折叠已展开的子菜单
            $et.parents(".jfgrid-body").find('.fa-minus').parent().not($et).click();
            var $cell = $et;
            var rownum = $et.attr('rownum');
            if (!rownum) {
                $cell = $et.parents('.jfgrid-data-cell');
    ...
    $.fn.jfGrid = function (op) {
        ...
        var dfop = {
            ...
            removeSelected: false,            //是否取消选中行,默认否  王洪冈
    ...
    $.fn.jfGridSet = function (name, data) {
        ...
        switch (name) {
            case 'reload': // 重新加载一边数据
                ...
                if (dfop.removeSelected) {
                    dfop.running.rowSelected = null;//加载完后默认未选中行
                }
                break;
            case 'refresh':
                break;
            case 'refreshdata':
                ...
                break;
            case 'refreshheaddata':
                if (data) {
                    dfop.headData = data.headData || data;
                }
                _jfgrid.head($self, dfop);
                break;
            case 'addRow':
