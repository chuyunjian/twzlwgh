...
namespace Learun.Application.Web.Areas.LR_CodeGeneratorModule.Controllers
{
    ...
    public class TemplatePCController : MvcControllerBase
    {
        ...
        public ActionResult LookCustmerCode(string databaseLinkId, string dbTable, string formData, string queryData, string colData, string baseInfo)
        {
            ...
            // 详情类
            string detailCode = codeGenerator.DetailCreate(baseInfoObj, formDataObj, compontMap);
            // 详情类
            string detailJsCode = codeGenerator.DetailJsCreate(baseInfoObj, dbTableObj, formDataObj, compontMap2);
            var jsonData = new
            {
        ...
        public ActionResult CreateCustmerCode(string databaseLinkId, string dbTable, string formData, string queryData, string colData, string baseInfo, string moduleEntityJson)
        {
            ...
            foreach (var btn in colDataObj.btns)
            {
                switch (btn)
                {
                    ...
                    case "print":
                        ...
                        break;
                    case "detail":
                        ModuleButtonEntity detailsButtonEntity = new ModuleButtonEntity();
                        detailsButtonEntity.Create();
                        detailsButtonEntity.F_EnCode = "lr_detail";
                        detailsButtonEntity.F_FullName = "详情";
                        moduleButtonList.Add(detailsButtonEntity);
                        break;
                }
            }
            ...
            // 详情类
            codeContent = codeGenerator.DetailCreate(baseInfoObj, formDataObj, compontMap);
            codeGenerator.CreateDetailCodeFile(baseInfoObj, codeContent);
            // 详情js类
            codeContent = codeGenerator.DetailJsCreate(baseInfoObj, dbTableObj, formDataObj, compontMap2);
            codeGenerator.CreateDetailJSCodeFile(baseInfoObj, codeContent);
            return Success("创建成功");
        }
        #endregion
        ...
        public ActionResult LookFastCode(CodeBaseConfigModel codeBaseConfigModel)
        {
            ...
            // 详情类
            string detailCode = codeGenerator.DetailCreate(codeBaseConfigModel);
            // 详情js类
            string detailJsCode = codeGenerator.DetailJsCreate(codeBaseConfigModel);
            var jsonData = new
            {
                entityCode = entityCode,
                mapCode = mapCode,
        ...
        public ActionResult CreateFastCode(CodeBaseConfigModel codeBaseConfigModel)
        {
            ...
            // 详情类
            codeContent = codeGenerator.DetailCreate(codeBaseConfigModel);
            codeGenerator.CreateDetailCodeFile(codeBaseConfigModel, codeContent);
            // 详情js类
            codeContent = codeGenerator.DetailJsCreate(codeBaseConfigModel);
            codeGenerator.CreateDetailJSCodeFile(codeBaseConfigModel, codeContent);
            ...
            deleteButtonEntity.F_FullName = "删除";
            moduleButtonList.Add(deleteButtonEntity);
            ModuleButtonEntity detailButtonEntity = new ModuleButtonEntity();
            detailButtonEntity.Create();
            detailButtonEntity.F_ActionAddress = "/" + codeBaseConfigModel.area + "/" + codeBaseConfigModel.name + "/Detail";
            detailButtonEntity.F_EnCode = "lr_detail";
            detailButtonEntity.F_FullName = "详情";
            moduleButtonList.Add(detailButtonEntity);
