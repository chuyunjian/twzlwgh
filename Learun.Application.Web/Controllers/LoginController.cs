...
namespace Learun.Application.Web.Controllers
{
    ...
    public class LoginController : MvcControllerBase
    {
        ...
        public ActionResult GetUserInfo()
        {
            ...
            data.password = null;
            data.secretkey = null;
            Sys_AccessoriesIBLL bll = new Sys_AccessoriesBLL();
            JObject queryjson = new JObject();
            queryjson.Add("OperationCode", "UserHead");
            queryjson.Add("OperationID", LoginUser.userId);
            var list = (List<Sys_AccessoriesEntity>)bll.GetList(queryjson.ToString());
            if (list.Count > 0)
            {

                data.headImg = list[0].getHttpPath();
            }
            else
            {

                data.headImg = Config.GetValue("HeadDefault");
            }
            return Success(data);
        }
        #endregion
        ...
        public ActionResult CheckLogin(string username, string password, string verifycode)
        {
            //验证是否授权
            int UserCount = 1;
            bool IsCkeck =Learun.Util.Config.GetValue("SoftwareAauthorization").ToBool();
            if (IsCkeck)
            {
                if (LicenseHelper.CheckLicense(out UserCount) != -10000)
                {
                    return Fail("软件未授权:请联系厂家!");
                }
                else
                {
                    //获取系统用户数量,系统用户数量大于授权用户数量  

                    int tCount = userBll.GetUserTotalCount();
                    if (tCount > UserCount)
                    {
                        return Fail("软件用户数量已超过授权数:请联系厂家!");
                    }
                }
            }
