...
namespace Learun.Application.Web.Areas.LR_AuthorizeModule.Controllers
{
    ...
    public class UserRelationController : MvcControllerBase
    {
        ...
        [HttpGet]
        [AjaxOnly]
        public ActionResult GetObjectIdList(string userId, int category)
        {
            var data = userRelationIBLL.GetObjectIdList(userId, category);
            return Success(data);
        }
        #endregion
        ...
        [ValidateAntiForgeryToken]
        [AjaxOnly]
        public ActionResult SaveObjectList(string objectIds, int category, string userId)
        {
            userRelationIBLL.SaveObjectList(objectIds, category, userId);
            return Success("保存成功！");
        }
        #endregion
    }
